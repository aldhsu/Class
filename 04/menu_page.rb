class MenuPage
  ...
  def input_sanitation(message)
    ...
    loop do
      begin
        ...
        unless (output == 'q') || (output == 'a') || (output == 'r') || (output == 'b') || (output == 's')
          output = Integer(output)
        else
          return output
        end
      rescue
        puts "Input needs to something valid (Int or Q or A or R or B or S)"
      end
  ...
  def display_options(array)
    puts `clear`
    puts "Estate Management Database"
    puts "======================================="
    puts "No. \t Description"
    array.each_with_index do |item, index|
      puts "#{index}. \t #{item.to_s}"
    end
    puts "(A)dd (Q)quit (R)emove (B)ack (S)earch"
  end
  #return new menu object or do (String) other operation
  ...
  def add
    parameters = {}
    attributes = (@object_array[0].methods - Object.methods).select{|x| x.to_s.include?("=")}
    attributes.each do |symbol|
      puts "What is the #{symbol}?"
      parameters[symbol] = gets.chomp
    end
    new_object = @object_array[0].class.new(*parameters)
